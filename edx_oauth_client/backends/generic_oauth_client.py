...
DEFAULT_AUTH_PIPELINE = [
    ...
    'edx_oauth_client.pipeline.parse_user_information',
    ...
    'edx_oauth_client.pipeline.verify_user_email',
...
class GenericOAuthBackend(BaseOAuth2):
    ...
    def request_access_token(self, *args, **kwargs):
        return {'access_token': self.user_data(*args, **kwargs).get('access_token')}

    def user_data(self, *args, **kwargs):
       """
       Grab user profile information from SSO.
       """
       data = get_user_data(self.data.get('id_token'))
       data['access_token'] = data.get('socialIdpUserId')
       return data
